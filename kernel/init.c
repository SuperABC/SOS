...
void machine_info() {
    ...
    col = cursor_col;
    cursor_row = 29;
    kernel_printf("%s", "Created by SGP, SuperSix, SBJ at Zhejiang University.");
    cursor_row = row;
    cursor_col = col;
    kernel_set_cursor();
}
...
void create_startup_process() {
    ...
    pc_create(1, desktop, (unsigned int)kmalloc(4096) + 4096, init_gp, "desktop"); //desktop
    log(LOG_OK, "Desktop init");
    pc_create(2, system_time_proc, (unsigned int)kmalloc(4096) + 4096, init_gp, "time"); //time
