...
void create_startup_process() {
    ...
    pc_create(1, ps, (unsigned int)kmalloc(4096) + 4096, init_gp, "powershell"); //创建powershell进程
    log(LOG_OK, "Shell init");
    pc_create(2, system_time_proc, (unsigned int)kmalloc(4096) + 4096, init_gp, "time"); //创建time进程
